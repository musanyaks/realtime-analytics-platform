  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "hourly",
        fluidRow(box(DTOutput("hourly_table"), width = 12, title = "Hourly Performance")),
        fluidRow(
      ...
      tabItem(tabName = "users",
        ...
        fluidRow(box(DTOutput("top_users_table"), width = 12, title = "Top Users by LTV"))
      ),
      tabItem(tabName = "quality",
        fluidRow(box(DTOutput("pipeline_status"), width = 12, title = "Pipeline Monitoring"))
      )
    )
  )
)
...
server <- function(input, output, session) {
  ...
  load_data <- function() {
    ...
    hourly <- dbGetQuery(con, "SELECT * FROM PROD.FCT_HOURLY_METRICS WHERE event_date >= DATEADD(day, -7, CURRENT_DATE()) ORDER BY event_date DESC, event_hour DESC LIMIT 168")
    users <- dbGetQuery(con, "SELECT * FROM PROD.DIM_USERS ORDER BY lifetime_value DESC LIMIT 100")
    monitoring <- dbGetQuery(con, "SELECT * FROM RAW.PIPELINE_MONITORING")
    list(hourly = hourly, users = users, monitoring = monitoring)
  }

  observe({ data(load_data()) })
  observeEvent(input$refresh, { data(load_data()) })

  autoInvalidate <- reactiveTimer(60000)
  observe({ autoInvalidate(); data(load_data()) })

  output$total_events_box <- renderValueBox({
    req(data())
    total <- sum(data()$hourly$total_events, na.rm = TRUE)
    valueBox(scales::comma(total), "Total Events (7d)", icon = icon("bolt"), color = "yellow")
  })

  output$revenue_box <- renderValueBox({
    req(data())
    rev <- sum(data()$hourly$revenue, na.rm = TRUE)
    valueBox(scales::dollar(rev), "Revenue (7d)", icon = icon("dollar-sign"), color = "green")
  })

  output$users_box <- renderValueBox({
    req(data())
    users <- sum(data()$hourly$unique_users, na.rm = TRUE)
    valueBox(scales::comma(users), "Unique Users (7d)", icon = icon("users"), color = "aqua")
  })

  output$conversion_box <- renderValueBox({
    req(data())
    conv <- weighted.mean(data()$hourly$conversion_rate, data()$hourly$unique_sessions, na.rm = TRUE)
    valueBox(scales::percent(conv, accuracy = 0.1), "Avg Conversion", icon = icon("chart-line"), color = "purple")
  })
  ...
  output$events_trend <- renderPlotly({
    ...
    data()$hourly %>%
      ...
      layout(paper_bgcolor = "#27293d", plot_bgcolor = "#27293d", font = list(color = "#fff"),
             xaxis = list(title = "", gridcolor = "#444"), yaxis = list(title = "Events", gridcolor = "#444"))
  })
  ...
  output$device_breakdown <- renderPlotly({
    ...
    data()$hourly %>%
      ...
      plot_ly(labels = ~c("Mobile", "Desktop/Tablet"), values = ~c(mean(mobile), 1 - mean(mobile)),
              type = "pie", marker = list(colors = c("#00d2ff", "#ff6b6b"))) %>%
      layout(paper_bgcolor = "#27293d", font = list(color = "#fff"), showlegend = FALSE)
  })

  output$hourly_table <- renderDT({
    req(data())
    datatable(data()$hourly, options = list(pageLength = 24), class = "cell-border stripe")
  })

  output$hourly_revenue <- renderPlotly({
    req(data())
    data()$hourly %>%
      plot_ly(x = ~event_hour, y = ~revenue, type = "bar", marker = list(color = "#00d2ff")) %>%
      layout(paper_bgcolor = "#27293d", plot_bgcolor = "#27293d", font = list(color = "#fff"),
             xaxis = list(title = "Hour", gridcolor = "#444"), yaxis = list(title = "Revenue", gridcolor = "#444"))
  })

  output$hourly_conversion <- renderPlotly({
    req(data())
    data()$hourly %>%
      plot_ly(x = ~event_hour, y = ~conversion_rate, type = "bar", marker = list(color = "#ff6b6b")) %>%
      layout(paper_bgcolor = "#27293d", plot_bgcolor = "#27293d", font = list(color = "#fff"),
             xaxis = list(title = "Hour", gridcolor = "#444"), yaxis = list(title = "Conversion Rate", gridcolor = "#444"))
  })
  ...
  output$top_countries <- renderPlotly({
    ...
    data()$users %>%
      ...
      arrange(desc(n)) %>%
      head(10) %>%
      plot_ly(x = ~most_common_country, y = ~n, type = "bar", marker = list(color = "#feca57")) %>%
      layout(paper_bgcolor = "#27293d", plot_bgcolor = "#27293d", font = list(color = "#fff"))
  })

  output$top_users_table <- renderDT({ req(data()); datatable(data()$users, options = list(pageLength = 10)) })
  output$pipeline_status <- renderDT({ req(data()); datatable(data()$monitoring, options = list(pageLength = 10)) })
}
